...
**Proof idea:** Permuting hidden neurons preserves the input-output map:
...
**Proof idea:** Taylor with Lagrange remainder on $g(t) := f(w_1 + t(w_2-w_1))$:

$$g(1) = g(0) + g'(0) + \frac12 g''(\xi)$$

for some $\xi\in(0,1)$; bound the second-order term via Lem 1.
...
**Thm 3 (no bad local minima for LNNs; Laurent & Brecht).** Let $\ell:\mathbb{R}^{d_N,d_0}\to\mathbb{R}$ be a differentiable **convex** loss inducing $\phi(W_1,\dots,W_N) := \ell(W_N\cdots W_1)$. Assume no bottleneck:

$$\min_{i\in[N-1]} d_i \ge \min\{d_0, d_N\}$$

Then any local minimizer $(\hat W_1,\dots,\hat W_N)$ of $\phi(\cdot)$ is a global minimizer.

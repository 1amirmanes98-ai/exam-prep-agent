...
, with $x,\mathbf{y}\in\mathbb{R}^m$, $x\ne0$, $\langle x,\mathbf{y}\rangle>0$.

**P2.** $\phi$ is non-convex.
Technique: exhibit a critical point that is not a global minimum: chain rule gives

$$\frac{\partial\phi}{\partial w_j} = \nabla L_S\big(\prod_i w_i\big)\prod_{k\ne j}w_k$$

, so $\nabla\phi(0)=0$ (for $N\ge2$); but $L_S$ is convex with unique minimizer
...
Technique: rescaling symmetry of the product parameterization: for every $c>0$,

$$\theta_c := (cw^*, \tfrac1c, 1,\dots,1)^\top$$

satisfies $\phi(\theta_c) = L_S(w^*)$, hence a continuum of global minimizers.

**P4.** For $N=2$: no bad local minima and no non-strict saddles.
Technique: critical points satisfy $\nabla L_S(w_1w_2)=0$ (then $(w_1,w_2)$ is a global min since $w_1w_2$ minimizes $L_S$) or $w_1=w_2=0$. At $(0,0)$ compute the Hessian:
...
exhibits a negative eigenvalue $\Rightarrow$ strict saddle.
...
Technique: conservation law — chain rule gives

$$\frac{d}{dt}w_1(t)^2 = -2w_1w_2\nabla L_S(w_1w_2) = \frac{d}{dt}w_2(t)^2$$

, so $w_1(t)^2 - w_2(t)^2 \equiv c$ ("balancedness conservation"). Combine with $w_1(\infty)w_2(\infty) = w^*$: solve the quadratic in $w_2(\infty)^2$ to get
...
**P7.** Theorem 1 (generalization bound): for any $\delta\in(0,1)$, w.p. $\ge 1-\delta$ over $S\sim\mathcal{D}^m$,

...
const FIGS = {
  vi_contraction: {
    title: "איטרציית ערך היא כיווץ-γ",
    pillar: "Planning",
    caption: "מחושב: איטרציית ערך על MDP קבוע עם 3 מצבים ו-2 פעולות (γ=0.9). שגיאת נורמת-המקסימום ‖Vₖ−V*‖∞ (סגול) נשארת מתחת למעטפת γᵏ‖V₀−V*‖∞ (מקווקו) בכל צעד — הכיווץ שגורם לאיטרציית הערך להתכנס גאומטרית (הרצאה 4).",
    draw(cv) {
  ...
  td_learning: {
    title: "TD(0) לומד את פונקציית הערך",
    pillar: "Learning",
    caption: "מחושב: TD(0) על הילוך אקראי בן 5 מצבים (γ=1, α=0.1, זרע קבוע). חמשת האומדים V(s) (טורקיז) מטפסים מ-0 לעבר ערכיהם האמיתיים s/6 (מקווקו) ככל שמצטברות אפיזודות — חיזוי חסר-מודל, bootstrapping ממעברים שנדגמו (הרצאות 6–7).",
    draw(cv) {
      const W = 640, H = 300, ctx = figCanvas(cv, W, H);
  ...
  gridworld_policy: {
    title: "המדיניות האופטימלית על GridWorld",
    pillar: "Planning",
    caption: "מחושב: איטרציית ערך על GridWorld בגודל 5×5 (γ=0.9; המטרה בפינה הימנית-עליונה = +1 בולעת; תא חסום). התאים מוצללים לפי V*; החיצים הם הפעולה האופטימלית החמדנית argmaxₐ Q*(s,a) — תכנון דינמי שהופך ערכים למדיניות (הרצאות 3–4).",
    draw(cv) {
  ...
  ucb_regret: {
    title: "החרטה של UCB גדלה רק כמו log T",
    pillar: "Bandits",
    caption: "מחושב: UCB1 על 5 זרועות ברנולי (תוחלות .50/.55/.60/.63/.65, זרע קבוע). החרטה המצטברת Σ(μ*−μₐₜ) (כתום) קעורה — תת-לינארית, ≈ log T — הרבה מתחת לקו הייחוס הלינארי המקווקו של 'המשך לחקור'. איזון בין חקירה לניצול (הרצאה 10).",
    draw(cv) {
      const W = 640, H = 300, ctx = figCanvas(cv, W, H);
  ...
  mdp_diagram: {
    title: "MDP כתרשים מעברים",
    pillar: "Planning",
    caption: "האובייקטים שמהם בנוי תהליך החלטה מרקובי: מצבים (עיגולים), פעולות (קשתות צבעוניות), כל קשת מסומנת בהסתברות המעבר ובתגמול המיידי שלה P(s'|s,a) / r. ממצב s₁ פעולה a (סגול) מתפצלת 0.8/0.2; פעולה b (כתום) היא מעבר ודאי. s₂ בולע. כל שיטת תכנון — אופרטורי בלמן, איטרציית ערך ומדיניות — פועלת בדיוק על המבנה הזה (הרצאות 2–3).",
    draw(cv) {
      const W = 640, H = 300, ctx = figCanvas(cv, W, H);
  ...
  policy_gradient: {
    title: "מדיניות-גרדיאנט מטפסת בתוחלת ההחזר",
    pillar: "Approximation",
    caption: "מחושב: עליית גרדיאנט על מדיניות softmax מעל שתי פעולות (תגמולים 1.0 מול 0.3), לפי משפט מדיניות-גרדיאנט ∇J = E[r·∇log π]. תוחלת ההחזר J(θ) (כתום) עולה מונוטונית עד לאופטימום (מקווקו) ככל שהמדיניות מסיטה הסתברות אל הפעולה הטובה יותר — בלי מודל, רק אומד פונקציית-הציון (הרצאה 9, REINFORCE / actor-critic).",
    draw(cv) {
      const W = 640, H = 300, ctx = figCanvas(cv, W, H);
...
// Regexes match topic / cheat-sheet / solution text, which is Hebrew on this site —
// so each keeps the Latin terms that survive translation (MDP, TD, UCB, bandit,
// REINFORCE, Bellman, Q-learning) AND adds Hebrew synonyms for the translated concepts.
const FIG_TOPIC_MAP = [
  [/value iteration|contraction|bellman operator|איטרציית ערך|כיווץ|אופרטור בלמן/i, "vi_contraction"],
  [/temporal difference|td\(|monte.?carlo|model-free|q-learning|הפרש זמני|מונטה.?קרלו|חסר.?מודל/i, "td_learning"],
  [/mdp formal|optimal policy|policy iteration|grid|dynamic programming|מדיניות אופטימלית|איטרציית מדיניות|תכנון דינמי/i, "gridworld_policy"],
  [/bandit|ucb|regret|exploration|best.?arm|שודד|חרטה|חקירה|זרוע/i, "ucb_regret"],
  [/mdp|markov decision|transition|state.?space|reward function|מרקוב|מעבר|מרחב המצבים|פונקציית תגמול/i, "mdp_diagram"],
  [/policy gradient|reinforce|actor.?critic|score function|function approximation|מדיניות-גרדיאנט|קירוב פונקציות|אקטור.?קריטיק/i, "policy_gradient"],
];
const FIG_MEMO_MAP = [
  [/value iteration|policy iteration|איטרציית ערך|איטרציית מדיניות/i, "vi_contraction"],
  [/bellman optimality operator|אופרטור בלמן/i, "vi_contraction"],
  [/td\(0\)|temporal difference|monte-carlo|q-learning|הפרש זמני|מונטה.?קרלו/i, "td_learning"],
  [/discounted mdp|optimal policy|מדיניות אופטימלית|מהוון/i, "gridworld_policy"],
  [/ucb|regret|best-arm|שודד|חרטה|זרוע/i, "ucb_regret"],
  [/mdp|markov decision|transition|מרקוב|מעבר/i, "mdp_diagram"],
  [/policy gradient|reinforce|actor-critic|score function|מדיניות-גרדיאנט|קירוב פונקציות/i, "policy_gradient"],
];
